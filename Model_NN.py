...
def Model_NN(train_data, train_target, test_data, test_target, sol=0.85):

    train_data = np.resize(train_data, (train_data.shape[0], 100))
    test_data = np.resize(train_data, (test_data.shape[0], 100))

    # train_data = (((train_data - np.min(train_data)) * (1 - 0)) / (np.max(train_data) - np.min(train_data)))
    pred = np.zeros(test_target.shape)
    for i in range(train_target.shape[1]):
        out, model2 = train_nn(train_data, train_target[:, i], test_data, 30)  # DNN
