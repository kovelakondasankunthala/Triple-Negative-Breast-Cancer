...
def MTBO(pop_Position, fobj, lb, ub, Max_iteration):
    ...
    for it in range(MaxIt):
        ...
        for i in range(nPop):
            ...
            if np.random.rand() < Li:
                newsol_Position = pop_Position[i] + np.multiply(np.random.rand(N, dim), (pop_Position[i] - pop_Position[i])) + np.multiply(np.random.rand(N, dim), (Leader_Position - pop_Position[i]))
            else:
                if np.random.rand() < Ai:
                    newsol_Position = pop_Position[i] + np.multiply(1 * np.random.rand(N, dim),(pop_Position[i] - pop_Position[nPop-1]))
                else:
                    if np.random.rand() < Mi:
                        newsol_Position = pop_Position[i] + np.multiply(1 * np.random.rand(N, dim), (Mean - pop_Position[i]))
                    else:
